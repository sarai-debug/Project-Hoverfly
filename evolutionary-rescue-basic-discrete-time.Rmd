...
-   `s_f` fecundity advantage for adults
-   `s_E` fecundity advantage for eggs
-   `s_E_0` baseline survival rate of eggs -\> adult, in stress-free environment
-   `r_A` reduction in adult fecundity due to heat
-   `r_E` reduction in eggs survival due to heat
...
A_w <- A_w_0
A_m <- A_m_0
one_generation <- function(A_w, A_m, f_w, f_m, s_E_0, r_A, r_E, mu_rate){
  # draw offspring according to Poisson distribution
  E_w <- rpois(1, A_w * (1-decay_rate))
  E_m <- rpois(1, A_m * (1-decay_rate+sel_coeff))
  # draw new mutants according to Poisson distribution
  mut_a_to_A <- rpois(1, offsp_a * mut_rate)
