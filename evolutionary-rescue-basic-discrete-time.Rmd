...
simulate_pop <- function(Z_init_w, Z_init_m, f_w, f_m, hZ_w, hZ_m, pZ_w, pZ_m, mut_rate, t_max) {
  ...
  for (i in 1:t_max+1) {
    ...
    if (pop_new[1]+pop_new[2]>=1000*(Z_init_w+ Z_init_m) | pop_new[1]+pop_new[2]==0) break
  }
...
output <- simulate_pop(Z_init_w = 100, Z_init_m = 50, f_w = 3, f_m = 3, hZ_w = 0.5, hZ_m = 0.5, pZ_w = 0.5, pZ_m = 0.5, mut_rate = 0.01,t_max = max_gen)
