...
simulate_one_gen_ls <- function(Z_w, Z_m, f_w, f_m, hZ_w, hZ_m, pZ_w, pZ_m, mut_rate) {
   ...
   Z_w_new <- rbinom(1,Z_w - newAdults_w + newEggs_w - mut_w_to_m, pZ_w)

   Z_m_new <- rbinom(1,Z_m - newAdults_m + newEggs_m + mut_w_to_m, pZ_m)
...
output <- simulate_pop(Z_init_w = 100, Z_init_m = 50, f_w = 3, f_m = 3, hZ_w = 0.5, hZ_m = 0.5, pZ_w = 0.5, pZ_m = 0.5, mut_rate = 0.001,t_max = max_gen)
...
for (i in 1:num_rounds) {
  ...
  round_gen <- simulate_pop(Z_init_w = 1, Z_init_m = 100, f_w = 3, f_m = 3, 
                            hZ_w = 0.5, hZ_m = 0.5, pZ_w = 0.5, pZ_m = 0.5, 
                            mut_rate = 0.01,t_max = max_gen)
...
```{r}
mut_rates <- seq(0.01, 0.5, by = 0.02)
...
for (i in 1:num_rounds) {
  ...
  for (mut in seq(0.01, 0.5, by = 0.02)){
