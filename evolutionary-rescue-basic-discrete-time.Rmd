...
-   `f_w` fecundity wild-type adults, eggs laid per adult per generation
-   `f_m` fecundity mutated adults, eggs laid per adult per generation
...
## New code 
```{r}
simulate_one_gen_ls <- function(Z_w, Z_m, f_w, f_m, hZ_a, hZ_A, pZ_a, pZ_A, mut_rate) {
   ...
   newAdults_a <- rbinom(1, Z_w,  hZ_a)

   newAdults_A <- rbinom(1, Z_m,  hZ_A)

   # adults produce eggs with reproduction rate r

   newEggs_a <- rpois(1, newAdults_a * f_w)

   newEggs_A <- rpois(1, newAdults_A * f_m)

   # draw new mutants according to Poisson distribution

   mut_a_to_A <- rpois(1, newEggs_a * mut_rate)

   # determine new population sizes of eggs, taking into account mortality during winter

   Z_a_new <- rbinom(1,Z_w - newAdults_a + newEggs_a - mut_a_to_A, pZ_a)

   Z_A_new <- rbinom(1,Z_m - newAdults_A + newEggs_A + mut_a_to_A, pZ_A)

   return(c(Z_a_new, Z_A_new))

 }

 # Test the function

 print(simulate_one_gen_ls(Z_w=1000, Z_m=0, f_w=2, f_m=2.5, hZ_a=0.5, hZ_A=0.5, pZ_a=0.1, pZ_A=0.1, mut_rate=0.1))
